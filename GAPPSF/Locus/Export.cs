    ...
    public class Export
    {
        public static void ExportToFile(string filename, List<Core.Data.Geocache> gcList)
        {
            try
            {
                ...
                using (Utils.ProgressBlock fixscr = new Utils.ProgressBlock("ExportingLocus", "CreatingFile", 1, 0, true))
                {
                    ...
                    Utils.ResourceHelper.SaveToFile("/Locus/sqlite.db3", filename, true);

                    SqliteConnection dbconFiles = null;
                    string basePath = null;
                    int imgFolderIndex = 0;
                    int imgInFolderCount = 0;
                    if (Core.Settings.Default.LocusExportOfflineImages)
                    {
                        basePath = System.IO.Path.GetDirectoryName(filename);
                        basePath = System.IO.Path.Combine(basePath, ".GrabbedImages");
                        if (!System.IO.Directory.Exists(basePath))
                        {
                            System.IO.Directory.CreateDirectory(basePath);
                        }
                        if (Core.Settings.Default.LocusMaxImagesInFolder > 0)
                        {
                    ...
                    using (SqliteConnection dbcon = new SqliteConnection(string.Format("data source=file:{0}", filename)))
                    {
                        ...
                        DbParameter par;
                        using (Utils.ProgressBlock progress = new Utils.ProgressBlock("ExportingLocus", "SavingGeocaches", gcList.Count, 0, true))
                        {
                            ...
                            using (SqliteCommand cmd8 = new SqliteCommand("", dbcon))
                            {
                                ...
                                foreach (Core.Data.Geocache gc in gcList)
                                {
                                    ...
                                    List<Core.Data.Log> logs = Utils.DataAccess.GetLogs(gc.Database, gc.Code).Take(Core.Settings.Default.LocusMaxLogs).ToList();
                                        ...
                                        try
                                        {
                                            ...
                                            foreach (string im in imgs)
                                            {
                                                if (oimgs.ContainsKey(im))
                                                {
                                                    ...
                                                    {
                                                        imgInFolderCount++;
                                                        if (imgInFolderCount > Core.Settings.Default.LocusMaxImagesInFolder)
                                                        {
                                                            imgFolderIndex++;
                                                            imgInFolderCount = 1;
                                                        }
