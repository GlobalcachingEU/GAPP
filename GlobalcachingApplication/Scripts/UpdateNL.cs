    ...
    private void threadMethod()
    {
        try
        {
            ...
            using (ProgressBlock progress = new ProgressBlock(_plugin, "Bijwerken van status en nieuwe geocaches", "Download gegevens van globalcaching.eu", 1, 0, true))
            {
                ...
                {
                    string doc = wc.DownloadString(string.Format("https://www.4geocaching.eu/Service/GeocacheCodes.aspx?country=Netherlands&token={0}", System.Web.HttpUtility.UrlEncode(_core.GeocachingComAccount.APIToken)));
                    if (doc != null)
                    {
