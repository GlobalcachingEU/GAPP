...
class Script
{
    ...
    private void threadMethod()
    {
        ...
        {
            bool cancel = false;
            using (ProgressBlock progress = new ProgressBlock(_plugin, "Update status of geocaches and import new geocaches", "Downloading data from globalcaching.eu", 1, 0, true))
            {
                using (System.Net.WebClient wc = new System.Net.WebClient())
                {
                    string doc = wc.DownloadString(string.Format("https://www.4geocaching.eu/Service/GeocacheCodes.aspx?country=Luxembourg&token={0}", System.Web.HttpUtility.UrlEncode(_core.GeocachingComAccount.APIToken)));
                    if (doc != null)
                    {
                        List<string> gcList = new List<string>();

                        string[] lines = doc.Replace("\r", "").Split(new char[] { '\n' });
                        progress.UpdateProgress("Update status of geocaches and import new geocaches", "Updating the status...", lines.Length, 0);
                        Geocache gc;
                        char[] sep = new char[] { ',' };
                        string[] parts;
                        ...
                        if (gcList.Count == 0)
                        {
                            System.Windows.Forms.MessageBox.Show("There are no new geocaches found", "Message", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information);
                        }
                        else
                        {
                            if (System.Windows.Forms.MessageBox.Show(string.Format("There are {0} new geocaches found\r\nImport these?", gcList.Count), "Message", System.Windows.Forms.MessageBoxButtons.YesNo, System.Windows.Forms.MessageBoxIcon.Information) == System.Windows.Forms.DialogResult.Yes)
                            {
                                progress.UpdateProgress("Bijwerken van status en nieuwe geocaches", "Importing geocaches...", gcList.Count, 0);
                                ...
                                using (GeocachingLiveV6 client = new GeocachingLiveV6(_core, string.IsNullOrEmpty(_core.GeocachingComAccount.APIToken)))
                                {
                                    ...
                                    while (gcList.Count > 0)
                                    {
                                        ...
                                        if (!progress.UpdateProgress("Update status of geocaches and import new geocaches", "Importing geocaches...", total, index))
                                        {
                                            break;
                                        }
                                    }
