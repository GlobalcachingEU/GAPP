...
namespace GlobalcachingApplication.Plugins.LanguageDutch
{
    public class LanguageSupport: Utils.BasePlugin.BaseLanguageSupport
    {
        ...
        public override string FriendlyName
        {
            get
            {
                return "Nederlandse vertalingen";
            }
        }
        ...
        private void initDatabase()
        {
            ...
            using (StreamReader textStreamReader = new StreamReader(assembly.GetManifestResourceStream("GlobalcachingApplication.Plugins.LanguageDutch.LookupDictionary.xml")))
            {
            ...
            try
            {
                ...
                {
                    if (!Core.SettingsProvider.TableExists(Core.SettingsProvider.GetFullTableName("translation_nl")))
                    {
                        Core.SettingsProvider.Database.Execute(string.Format("create table '{0}' (item_name text, item_value text)", Core.SettingsProvider.GetFullTableName("translation_nl")));
                    }
                    List<DictionaryPoco> pocos = Core.SettingsProvider.Database.Fetch<DictionaryPoco>(string.Format("select * from {0}", Core.SettingsProvider.GetFullTableName("translation_nl")));
                ...
                if (uc is SettingsPanel)
                {
                    ...
                    try
                    {
                        ...
                        lock(Core.SettingsProvider)
                        {
                            foreach (DictionaryItem di in dict)
                            {
                                ...
                                if (string.IsNullOrEmpty(di.Value) || di.Value==s)
                                {
                                    ...
                                    {
                                        Core.SettingsProvider.Database.Execute(string.Format("delete from {0} where item_name='{1}'", Core.SettingsProvider.GetFullTableName("translation_nl"), di.Key.ToLower().Replace("'", "''")));
                                ...
                                else if (di.Value!=s && di.Value!=cs)
                                {
                                    ...
                                    if (Core.SettingsProvider.Database.Execute(string.Format("update {2} set item_value='{1}' where item_name='{0}'", di.Key.ToLower().Replace("'", "''"), di.Value.Replace("'", "''"), Core.SettingsProvider.GetFullTableName("translation_nl"))) == 0)
                                    {
                                        Core.SettingsProvider.Database.Execute(string.Format("insert into {2} (item_name, item_value) values ('{0}', '{1}')", di.Key.ToLower().Replace("'", "''"), di.Value.Replace("'", "''"), Core.SettingsProvider.GetFullTableName("translation_nl")));
                                    }
                                    changed = true;
                                }
                            }
...
#endif
                        {
                            ...
                            using (TextWriter sw = new StreamWriter(System.IO.Path.Combine(Core.PluginDataPath, "LanguageDutch.xml" ), false, Encoding.UTF8)) //Set encoding
                            {
                                doc.Save(sw);
                            }
                        }
