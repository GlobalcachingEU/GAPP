...
namespace GlobalcachingApplication.Plugins.LanguageFrench
{
    public class LanguageSupport: Utils.BasePlugin.BaseLanguageSupport
    {
        ...
        public async override Task<bool> InitializeAsync(Framework.Interfaces.ICore core)
        {
            ...
            if (await base.InitializeAsync(core))
            {
                ...
                li.Action = "Francais";
                li.CultureInfo = new System.Globalization.CultureInfo(1036);
        ...
        private void initDatabase()
        {
            ...
            using (StreamReader textStreamReader = new StreamReader(assembly.GetManifestResourceStream("GlobalcachingApplication.Plugins.LanguageFrench.LookupDictionary.xml")))
            {
            ...
            try
            {
                ...
                {
                    if (!Core.SettingsProvider.TableExists(Core.SettingsProvider.GetFullTableName("translation_fr")))
                    {
                        Core.SettingsProvider.Database.Execute(string.Format("create table '{0}' (item_name text, item_value text)", Core.SettingsProvider.GetFullTableName("translation_fr")));
                    }
                    List<DictionaryPoco> pocos = Core.SettingsProvider.Database.Fetch<DictionaryPoco>(string.Format("select * from {0}", Core.SettingsProvider.GetFullTableName("translation_fr")));
                ...
                if (uc is SettingsPanel)
                {
                    ...
                    try
                    {
                        ...
                        lock (Core.SettingsProvider)
                        {
                            foreach (DictionaryItem di in dict)
                            {
                                ...
                                if (string.IsNullOrEmpty(di.Value) || di.Value == s)
                                {
                                    ...
                                    {
                                        Core.SettingsProvider.Database.Execute(string.Format("delete from {0} where item_name='{1}'", Core.SettingsProvider.GetFullTableName("translation_fr"), di.Key.ToLower().Replace("'", "''")));
                                ...
                                else if (di.Value != s && di.Value != cs)
                                {
                                    ...
                                    if (Core.SettingsProvider.Database.Execute(string.Format("update {2} set item_value='{1}' where item_name='{0}'", di.Key.ToLower().Replace("'", "''"), di.Value.Replace("'", "''"), Core.SettingsProvider.GetFullTableName("translation_fr"))) == 0)
                                    {
                                        Core.SettingsProvider.Database.Execute(string.Format("insert into {2} (item_name, item_value) values ('{0}', '{1}')", di.Key.ToLower().Replace("'", "''"), di.Value.Replace("'", "''"), Core.SettingsProvider.GetFullTableName("translation_fr")));
                                    }
                                    changed = true;
                                }
                            }
...
#endif
                        {
                            ...
                            using (TextWriter sw = new StreamWriter(System.IO.Path.Combine(Core.PluginDataPath, "LanguageFrench.xml" ), false, Encoding.UTF8)) //Set encoding
                            {
                                doc.Save(sw);
                            }
                        }
