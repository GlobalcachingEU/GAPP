    ...
    public class InternalStorage : Utils.BasePlugin.BaseInternalStorage
    {
        ...
        void Geocaches_LoadFullData(object sender, Framework.EventArguments.LoadFullGeocacheEventArgs e)
        {
            if (_dbcon != null)
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand(string.Format("select shortdescr, shortdescrhtml, longdescr, longdescrhtml from geocache where Code='{0}'", e.RequestedForGeocache.Code.Replace("'", "''")), _dbcon))
        ...
        void Logs_LoadFullData(object sender, Framework.EventArguments.LoadFullLogEventArgs e)
        {
            if (_dbcon != null)
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand(string.Format("select tbcode, finderid, logtext, encoded from log where ID='{0}'", e.RequestForLog.ID.Replace("'", "''")), _dbcon))
                    using (SqlDataReader dr = cmd.ExecuteReader())
                        if (dr.Read())
                        {
