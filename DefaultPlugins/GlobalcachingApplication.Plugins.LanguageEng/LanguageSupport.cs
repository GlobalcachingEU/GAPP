...
namespace GlobalcachingApplication.Plugins.LanguageEng
{
    public class LanguageSupport: Utils.BasePlugin.BaseLanguageSupport
    {
        ...
        private void initDatabase()
        {
            ...
            try
            {
                ...
                {
                    if (!Core.SettingsProvider.TableExists(Core.SettingsProvider.GetFullTableName("translation_us")))
                    {
                        Core.SettingsProvider.Database.Execute(string.Format("create table '{0}' (item_name text, item_value text)", Core.SettingsProvider.GetFullTableName("translation_us")));
                    }
                    List<DictionaryPoco> pocos = Core.SettingsProvider.Database.Fetch<DictionaryPoco>(string.Format("select * from {0}", Core.SettingsProvider.GetFullTableName("translation_us")));
