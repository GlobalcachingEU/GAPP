...
namespace GlobalcachingApplication.Plugins.Sqlite
{
    public class InternalStorage: Utils.BasePlugin.BaseInternalStorage
    {
            ...
            using (System.Windows.Forms.SaveFileDialog dlg = new System.Windows.Forms.SaveFileDialog())
            {
                ...
                if (dlg.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    try
                    {
                        ...
                        using (SqliteConnection dbcon = new SqliteConnection(string.Format("data source=file:{0}", dlg.FileName)))
                        {
            ...
            using (System.Windows.Forms.SaveFileDialog dlg = new System.Windows.Forms.SaveFileDialog())
            {
                ...
                if (dlg.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    try
                    {
                        ...
                        SqliteConnection dbcon = new SqliteConnection(string.Format("data source=file:{0}", dlg.FileName));
                        if (InitDatabase(dbcon))
                        {
        ...
        public override bool Load(bool geocachesOnly)
        {
            ...
            if (_dbcon != null)
            {
                ...
                if (dr.Read())
                {
                    gcCount = (int)(int)dr["geocache"];
                    logCount = (int)(int)dr["log"];
                    logimgCount = (int)(int)dr["waypoint"];
                    wptCount = (int)(int)dr["logimage"];
                }
                    ...
                    using (Utils.ProgressBlock progress = new ProgressBlock(this, STR_LOADING, STR_LOADINGGEOCACHES, gcCount, 0))
                    {
                        ...
                        using (SqliteDataReader dr = cmd.ExecuteReader())
                            while (dr.Read())
                            {
                                ...
                                //gc.DistanceToCenter = (int)(int)dr["disttocent"];
                                //gc.AngleToCenter = (int)(int)dr["angletocent"];
                                gc.Available = (int)(int)dr["available"] != 0;
                                gc.Archived = (int)(int)dr["archived"] != 0;
                                gc.Country = (string)dr["country"];
                                gc.State = (string)dr["state"];
                                gc.GeocacheType = Utils.DataAccess.GetGeocacheType(Core.GeocacheTypes, (int)(int)dr["cachetype"]);
                                gc.PlacedBy = (string)dr["placedby"];
                                gc.Owner = (string)dr["owner"];
                                gc.OwnerId = (string)dr["ownerid"];
                                gc.Container = Utils.DataAccess.GetGeocacheContainer(Core.GeocacheContainers, (int)(int)dr["container"]);
                                gc.Terrain = (double)dr["terrain"];
                                gc.Difficulty = (double)dr["difficulty"];
                                gc.EncodedHints = (string)dr["encodedhints"];
                                gc.Url = (string)dr["url"];
                                gc.MemberOnly = (int)(int)dr["memberonly"] != 0;
                                gc.CustomCoords = (int)(int)dr["customcoords"] != 0;
                                ...
                                gc.Flagged = (int)(int)dr["flagged"] != 0;
                                gc.Locked = (int)(int)dr["locked"] != 0;
                                gc.Found = (int)(int)dr["found"] != 0;

                                gc.Favorites = (int)(int)dr["favorites"];
                                gc.Selected = (int)(int)dr["selected"] != 0;
