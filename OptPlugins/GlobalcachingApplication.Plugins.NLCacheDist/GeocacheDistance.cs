    ...
    public class GeocacheDistance : Utils.BasePlugin.BaseImportFilter
    {
        ...
        protected override void ImportMethod()
        {
            using (Utils.ProgressBlock progress = new Utils.ProgressBlock(this, STR_IMPORT, STR_DOWNLOAD, 1, 0))
            {
                ...
                {
                    wc.DownloadFile(string.Format("https://www.4geocaching.eu/service/cachedistance.aspx?country=Netherlands&prefix=GC&token={0}", System.Web.HttpUtility.UrlEncode(Core.GeocachingComAccount.APIToken)), tmp.Path);
